...
class COCODatasetVisualPrompt(dataset.Dataset):
    def __init__(self,ann_path,image_folder,cate_feat=None,
                 ivi_number=1,aug=False,extract_box=False,strong_aug=False,train=False,support_norm=False,return_mask=False,mixed_support_selection=False,val_cls_ids=None,region_detect=False,use_text=False,bg_feat=None,use_bg=True,max_support_len=81):
    ...
    def get_sample_ids(self):
        imgids = []
        for caid,matchids in self.coco.catToImgs.items():
            if len(matchids)>64:
                sampleids = random.sample(matchids,64)
            else:
                sampleids = matchids
    ...
    def __getitem__(self, index):
        ...
        try:
            query_np, (h0, w0), (h, w) = self.resize_retain_hwratio(index)
        except:
        ...
        selected_bboxes, selected_bboxes_catid = select_random_bbox_per_category(tag_bboxes,pos_catids,self._ivi_number)
...
def select_random_bbox_per_category(tag_bboxes, pos_catids,ivi_number):
    # 创建一个字典来存储每个类别的边界框
    category_bboxes = {}
    ...
    for cat_id, bboxes in category_bboxes.items():
        if bboxes:  # 确保该类别有至少一个边界框
            slboxx = random.sample(bboxes,min(len(bboxes),ivi_number))
            for box in slboxx:
