...
class COCODatasetAVG(dataset.Dataset):
    def __init__(self,ann_path,image_folder,dino_feats_folder=None,support_feat_dim=384,max_support_len=81,extra_shots=0,aug=False,strong_aug=False,train=False,support_norm=False,return_mask=False,mixed_support_selection=False,val_cls_ids=None,region_detect=False,use_text=False,bg_feat=None):
    ...
    def __getitem__(self, index):
        ...
        if self.support_norm:
            support_samples[0] = torch.ones(self.support_feat_dim)/torch.linalg.norm(torch.ones(self.support_feat_dim))
        else:
            support_samples[0] = torch.ones(self.support_feat_dim)
        if not self.training:
            ...
            for cls_coco in unique_elements:
                try:
            ...
            reverse_mapping = {(k+1):v for k,v in enumerate(unique_elements)}
            if 'voc' in self.image_folder.lower():
