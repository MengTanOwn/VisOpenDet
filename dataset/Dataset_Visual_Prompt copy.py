    ...
    def __getitem__(self, index):
        ...
        if type_rand == 0 and self.training:
            ...
            for index_ann,each in enumerate(image_ann):
                ...
                if (xyxy[2]-xyxy[0])*(xyxy[3]-xyxy[1])<1 or ratio>0.5:
                    continue
                tag_bboxes.append(xyxy)
        ...
        num_cls_neg = random.randint(1, self.max_support_len - len(set(pos_catids)))#负样本类别数据
        ...
        #drop some prompt boxes
        # keep = random.sample(range(len(tag_labels_list)),len(tag_labels_list)-random.randint(0,len(tag_labels_list)-1))
        # box_torch = box_torch[0][keep].unsqueeze(0)
        # tag_labels_list_tensor = torch.tensor(tag_labels_list)[keep]
