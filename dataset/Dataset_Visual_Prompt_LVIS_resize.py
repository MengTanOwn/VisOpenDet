...
class LVISDatasetVisualPrompt(dataset.Dataset):
    def __init__(self,ann_path,image_folder,cate_feat=None,aug=False,strong_aug=False,train=False,support_norm=False,return_mask=False,extract_box=False,mixed_support_selection=False,val_cls_ids=None,region_detect=False,use_text=False,bg_feat=None,use_bg=True,max_support_len=81):
        super(LVISDatasetVisualPrompt, self).__init__()
        ...
        if extract_box:
            self.image_ids = self.get_sample_ids()
        else:
            self.image_ids = list(sorted(self.lvis.imgs.keys()))
    ...
    def get_sample_ids(self):
        imgids = []
        for caid,matchids in self.lvis.cat_img_map.items():
            if len(matchids)>16:
                sampleids = random.sample(matchids,16)
            else:
                sampleids = matchids
            imgids.extend(sampleids)
        return list(sorted(set(imgids)))
    ...
    def resize_retain_hwratio(self, img_id):
        """Loads a single image by index, returning the image, its original dimensions, and resized dimensions."""
        try:
            image_name = self.lvis.imgs[img_id]['file_name']
        except:
            image_name = self.lvis.imgs[img_id]['coco_url'].split('/')[-1]
    ...
    def __getitem__(self, index):
        ...
        neg_cates = self.lvis.imgs[img_id]['neg_category_ids']
        ann_ids = self.lvis.get_ann_ids(img_ids=[img_id])
        image_ann = self.lvis.load_anns(ann_ids)
        
        copy_image_ann = copy.deepcopy(image_ann)
        if image_ann == []: return self.__getitem__(random.randint(0,len(self.image_ids)-1))
        
        try:
            query_np, (h0, w0), (h, w) = self.resize_retain_hwratio(img_id)
        except:
        ...
        for index_ann,each in enumerate(image_ann):
            if each["category_id"] in neg_cates:
                fcate = each["category_id"]
                print(f"cate id {fcate} is in neg cates {str(neg_cates)}")
                continue
