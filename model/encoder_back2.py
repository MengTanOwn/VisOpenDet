...
class HybridEncoder(nn.Module):
    ...
    def _gather_bs_support(self, outputs):
        ...
        return unique_support_imagee_cat,unique_supprot_text_cat
    ...
    def forward(self, feats,support_feat=None,query_mask=None,support_mask=None,text_features=None,vp=None,cross_vp=False):
        ...
        if support_feat!=None:
            ...
            # box_embeddings = [self._embed_boxes_add_feat(each['boxes'],each['input_image_size'],feats[-1][each_index]) for each_index,each in enumerate(vp)]
            #pad box_embeddings to the same length
            # max_len = max([len(each['boxes'][0]) for each in vp])
            # # max_len = self.max_support_len
            ...
            # box_embeddings = self.support_input_proj(box_embeddings)#存在BUG，无法进行跨图推理指标计算。
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                #         _,box_embeddings = self.fusion_layers[index](src_flatten,box_embeddings,query_mask_flatten,support_mask)
                # if cross_vp == False:
                ...
                #     Q = self.content_embedding.weight[None, :]
                #     #expand to the same size as support_feat
                #     Q = Q.expand(box_embeddings.shape[0], box_embeddings.shape[1], box_embeddings.shape[2])#可能存在问题，所有类别公用同一个权重？
                #     Q_ = self.cross_attention_vp(self.with_pos_embed(Q.transpose(0,1),box_embeddings.transpose(0,1)), memory.transpose(0,1), memory.transpose(0,1),query_mask_flatten)[0].transpose(0,1)
                #     Q = Q + self.cross_attention_vp_dropout(Q_)
                #     Q = self.cross_attention_vp_norm(Q)
                #     q = k = self.with_pos_embed(Q, box_embeddings)
                #     Q_, _ = self.self_attn(q, k, value=Q, attn_mask=None)
                #     Q = Q + self.dropout_post(Q_)
                #     box_embeddings = self.norm_post(Q)
        ...
        for idx in range(len(self.in_channels) - 1): 
            ...
            #     _,box_embeddings = self.fusion_layers[index](out_flatten,box_embeddings,query_mask_flatten,support_mask)
        ...
        if cross_vp == False:
            ...
            box_embeddings = torch.stack(box_embeddings,dim=0)
            normalized_box_ref = torch.stack(normalized_box_ref,dim=0)[:,0]
        ...
        #     for i in range(bs):
        #         unique_cates = torch.unique(corresponding_cates[i])
        #         cate_feats = corresponding_cates_feat[i]
        #         for cate in unique_cates:
        #             mask = corresponding_cates[i] == cate
        #             #pad mask
        #             mask = F.pad(mask, (0, max_len - len(mask)))
        #             try:
        #                 box_emb_cate = F.normalize(box_embeddings[i, mask], dim=-1).mean(dim=0)
        #                 box_emb_cate = F.normalize(box_emb_cate, dim=-1)
        #                 support_feat[i, cate] = box_emb_cate
        #                 # support_mask[i, cate] = True
        #                 cate_feat_i = cate_feats[cate.item()].to(torch.float32)
        #                 # cate_feat_i = torch.tensor(cate_feats[cate.item()]).to(support_feat.device).to(torch.float32)
        #                 # cate_feat_i = self.text_input_proj(cate_feat_i)
        #                 support_cate_feat[i, cate] = cate_feat_i
        #                 support_cate_mask[i, cate] = 1
        #             except Exception as e:
        #                 print(e)
        #                 exit()
                        # import pdb;pdb.set_trace()
            # support_cate_feat = self.text_input_proj(support_cate_feat)
            # support_mask = torch.logical_not(support_mask)
        # if cross_vp == False:
        #     corresponding_cates = [each['cates'] for each in vp]
        #     corresponding_cates_tatal = [item.item() for sublist in corresponding_cates for item in sublist]
        #     unique_cates =list(set(corresponding_cates_tatal))
        #     unique_catas_feat_dict = {v:[] for v in unique_cates}
        #     corresponding_cates_feat = [each['tag_cate_feat'] for each in vp]
        #     unique_cates_text_feat = merge_dicts_reduce(corresponding_cates_feat)
        #     for i in range(bs):
        #         unique_cates_I = torch.unique(corresponding_cates[i])
        #         # cate_feats_i = corresponding_cates_feat[i]
        #         for cate in unique_cates_I:
        #             cate = cate.item()
        #             if cate >= 100:
        #                 continue
        #             mask = corresponding_cates[i] == cate
        #             #pad mask
        #             mask = F.pad(mask, (0, max_len - len(mask)))
        #             unique_catas_feat_dict[cate].append(box_embeddings[i, mask].mean(dim=0))
        #     print('batch cates :',len(unique_cates))          
        #     for cate in unique_cates:
        #         # cate = cate
        #         unique_catas_feats = torch.stack(unique_catas_feat_dict[cate]).mean(dim=0)
        #         support_feat[:, cate] = unique_catas_feats
